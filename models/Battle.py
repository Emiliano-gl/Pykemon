...
class Battle:
    ...
    def print_winner(self):
        if self.pokemon1.current_hp <= 0 < self.pokemon2.current_hp:
            print(f"{self.pokemon2.name} won in {self.actual_turn} turns")

        elif self.pokemon2.current_hp <= 0 < self.pokemon1.current_hp:
            print(f"{self.pokemon1.name} won in {self.actual_turn} turns")
    ...
    def compute_damage(self, attack, pokemon1, pokemon2):
        ...
        final_damage = damage_without_modifier * self.compute_damage_modifier(attack, pokemon1, pokemon2)
        return final_damage
